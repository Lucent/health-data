#!/usr/bin/env python3
"""BJ. Search simple directional asymmetry in defended-fat-mass updates.

Hypothesis:
  the defended level rises toward higher fat mass more readily than it falls
  toward lower fat mass.

Keep appetite pressure fixed:
  pressure (cal/day) = 55 * (SP - FM)

Search families:
  - AsymEMA: different half-lives for upward vs downward SP movement
  - AsymEMA+DownCap: same, but cap only the downward daily SP movement
  - AsymFixed: different fixed speeds up vs down
"""
...
def make_asym_ema_rule(hl_up, hl_down, down_cap=None):
    ...
    def rule(sp, fm):
        gap = fm - sp
        if gap > 0:
            return sp + alpha_up * gap
        step = alpha_down * gap
        if down_cap is not None:
            step = max(step, -down_cap)
        return sp + step
...
def make_asym_fixed_rule(speed_up, speed_down):
    def rule(sp, fm):
        gap = fm - sp
        if gap > 0:
            return sp + min(gap, speed_up)
        return sp - min(-gap, speed_down)
...
def main():
    ...
    for hl_up in [20, 30, 45, 60, 80]:
        for hl_down in [80, 120, 160, 200, 260, 365, 520, 730]:
            rule = make_asym_ema_rule(hl_up, hl_down)
            sp = apply_rule_series(fm, rule)
            fit = eval_subject(sp, fm, df["surplus_90"].values, df["surplus_180"].values, mask)
            row = {
                "model": "AsymEMA",
                "params": f"uphl={hl_up},downhl={hl_down}",
                "subject_r90": fit["r90"],
    ...
    for hl_up in [20, 30, 45, 60, 80]:
        for hl_down in [120, 200, 365, 520, 730]:
            for down_cap in [0.01, 0.02, 0.03, 0.05, 0.08]:
                rule = make_asym_ema_rule(hl_up, hl_down, down_cap=down_cap)
                sp = apply_rule_series(fm, rule)
                fit = eval_subject(sp, fm, df["surplus_90"].values, df["surplus_180"].values, mask)
                row = {
                    "model": "AsymEMA+DownCap",
                    "params": f"uphl={hl_up},downhl={hl_down},dcap={down_cap}",
                    "subject_r90": fit["r90"],
                    "subject_r180": fit["r180"],
                }
                row.update(eval_trials(rule))
    ...
    for speed_up in [0.03, 0.05, 0.08, 0.10, 0.15]:
        for speed_down in [0.001, 0.003, 0.005, 0.01, 0.02]:
    ...
    print("=" * 96)
    print("DIRECTIONAL ASYMMETRY SEARCH")
    print("=" * 96)
