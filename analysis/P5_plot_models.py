...
from P3_interpolate_weight import CALORIMETRY, CAL_PER_LB
...
def fig1_full_trajectory(daily):
    ...
    plt.tight_layout()
    out = ROOT / "analysis" / "P5_plot_trajectory.png"
...
def fig2_zoom_windows(daily):
    ...
    plt.tight_layout()
    out = ROOT / "analysis" / "P5_plot_zoom_windows.png"
    plt.savefig(out, dpi=150)
    print(f"Saved {out}")
    plt.close()
...
def fig3_tdee(daily):
    ...
    comp_path = ROOT / "analysis" / "P2_daily_composition.csv"
    if comp_path.exists():
    ...
    plt.tight_layout()
    out = ROOT / "analysis" / "P5_plot_tdee.png"
    plt.savefig(out, dpi=150)
    print(f"Saved {out}")
    plt.close()
...
def fig4_residual(daily):
    ...
    plt.tight_layout()
    out = ROOT / "analysis" / "P5_plot_residual.png"
    plt.savefig(out, dpi=150)
    print(f"Saved {out}")
    plt.close()
