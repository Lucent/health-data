...
Produces:
  intake_foods.csv  — every food item with all 8 nutrients
  intake_daily.csv  — daily totals (from TOTAL rows)
"""
...
def run_extractor(script, files):
    """Run an extractor and return parsed rows."""
    if not files:
        return []
...
def main():
    ...
    for year in sorted(years):
        ...
        for source, script, file_list in [
            ...
            rows = run_extractor(script, file_list)
            for r in rows:
                date = r["date"]
